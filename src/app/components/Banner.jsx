...
export default function Banner({
  ...
  return (
    <section id="home" className="flex flex-col snap-start relative w-full min-h-screen overflow-hidden">
      <div
      ...
      <div className="relative min-h-screen grid grid-rows-[1fr_auto] lg:grid-rows-1 lg:grid-cols-2 gap-8 lg:gap-0">
        ...
        <motion.div
          ...
          className="relative z-10 flex flex-col items-center justify-center text-center lg:text-left px-4 sm:px-6 lg:px-8 xl:px-12 py-8 sm:py-12 lg:py-20 order-1"
        >
          <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl text-white drop-shadow-lg leading-tight sm:leading-snug">
            {title}
          </h1>

          <h2 className="mt-4 sm:mt-6 max-w-3xl text-sm xs:text-base sm:text-lg md:text-xl text-gray-200/90 leading-relaxed sm:leading-loose px-2 sm:px-0">
            {subtitle}
          </h2>
          ...
          <motion.div
            ...
            className="flex flex-col xs:flex-row gap-3 sm:gap-4 mt-6 sm:mt-8 w-full max-w-xs xs:max-w-sm sm:max-w-md"
          >
            <a
              href={buttonLeftHref}
              className={`${buttonFullStyle} ${hoverEffect} text-sm xs:text-base sm:text-lg px-4 xs:px-6 py-3 sm:py-4 flex-1 text-center`}
            >
              {buttonLeftText}
            </a>
            <a
              href={buttonRightHref}
              className={`${buttonOutlineStyle} ${hoverEffect} text-sm xs:text-base sm:text-lg px-4 xs:px-6 py-3 sm:py-4 flex-1 text-center`}
            >
              {buttonRightText}
            </a>
          </motion.div>
        ...
        {/* Coluna Direita - Estatísticas e Tecnologias */}
        <motion.div
          initial={{ y: -200, opacity: 0, rotate: -15, scale: 0.9 }}
          whileInView={{ y: 0, opacity: 1, rotate: 0, scale: 1 }}
          transition={{ duration: 1.5, type: "spring", stiffness: 80, damping: 12 }}
          viewport={{ once: true, amount: 0.5 }}
          className="flex items-center justify-center px-4 sm:px-6 py-8 sm:py-12 lg:py-20 order-2 lg:order-2"
        >
          <div className="w-full max-w-sm sm:max-w-md  flex flex-col items-center gap-6 sm:gap-8 px-4 sm:px-6 py-6 sm:py-8 rounded-2xl">
            {/* Título Experiências */}
            <h2 className="text-xl sm:text-2xl lg:text-3xl text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-rose-500 font-medium text-center">
              Experiências
            </h2>

            {/* Estatísticas */}
            <div className="w-full max-w-xs sm:max-w-sm flex flex-col items-center gap-4 sm:gap-6">
              <div className="grid grid-cols-2 gap-3 sm:gap-4 w-full">
                <div className="flex flex-col items-center rounded-xl p-3 sm:p-4 shadow backdrop-blur-sm">
                  <p className="text-2xl xs:text-3xl sm:text-4xl font-extrabold text-purple-700 mb-1">1+</p>
                  <p className="text-xs xs:text-sm sm:text-base text-white font-medium text-center leading-tight">
                    Ano de<br />Experiência
                  </p>
                </div>
                <div className="flex flex-col items-center rounded-xl p-3 sm:p-4 shadow backdrop-blur-sm">
                  <p className="text-2xl xs:text-3xl sm:text-4xl font-extrabold text-purple-700 mb-1">39K+</p>
                  <p className="text-xs xs:text-sm sm:text-base text-white font-medium text-center leading-tight">
                    Xícaras de Café<br />Tomadas
                  </p>
                </div>
              </div>
              <div className="flex flex-col items-center rounded-xl p-3 sm:p-4 shadow backdrop-blur-sm w-full sm:w-2/3">
                <p className="text-2xl xs:text-3xl sm:text-4xl font-extrabold text-purple-700 mb-1">10+</p>
                <p className="text-xs xs:text-sm sm:text-base text-white font-medium text-center leading-tight">
            ...
            {/* Tecnologias */}
            <div className="flex flex-col items-center gap-3 sm:gap-4 w-full mt-4 sm:mt-6 lg:mt-8">
              <p className="text-lg sm:text-xl lg:text-2xl text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-rose-500 font-medium text-center">
                Tecnologias:
              </p>
              <div className="flex gap-3 sm:gap-6 lg:gap-8 flex-wrap justify-center">
                ...
                >
                  <svg
                    className={`${buttonTecnologias} w-7 h-7 xs:w-8 xs:h-8 sm:w-10 sm:h-10`}
                    aria-hidden="true"
